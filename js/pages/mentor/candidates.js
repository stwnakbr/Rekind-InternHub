...
async function loadCandidates() {
  ...
  tbody.innerHTML = candidates.map(c => {
    ...
    const lockedByOther = !isUnavailable && c.interviewing_mentor_id && c.interviewing_mentor_id !== user.id;
    const lockedByMe    = !isUnavailable && c.interviewing_mentor_id && c.interviewing_mentor_id === user.id;

    let mentorBadge = '';
    if (isUnavailable) {
      mentorBadge = `<div style="font-size:11px;color:#6b7280;margin-top:3px">🚫 Sudah diterima di posisi lain</div>`;
    } else if (lockedByOther) {
      mentorBadge = `<div style="font-size:11px;color:#b45309;margin-top:3px">🔒 ${c.interviewing_mentor_name || 'Mentor lain'}</div>`;
    } else if (lockedByMe) {
      mentorBadge = `<div style="font-size:11px;color:var(--primary);margin-top:3px">✋ Ditangani oleh Anda</div>`;
    }

    return `
    <tr style="${isUnavailable ? 'opacity:0.5' : lockedByOther ? 'opacity:0.7' : ''}">
      <td>
      ...
      <td>
        <span class="status-badge status-${isUnavailable ? 'unavailable' : (c.status||'pending').toLowerCase()}">${isUnavailable ? 'Unavailable' : c.status || 'Pending'}</span>
      ...
      <td>
        <button class="btn btn-outline btn-sm btn-view-profile" data-id="${c.id}"
          ${isUnavailable ? 'disabled title="Kandidat sudah diterima di posisi lain"' : ''}>
          👁 Lihat Profil
        </button>
      </td>
    </tr>`;
...
function openCandidateProfileModal(c) {
  ...
  const stepColors = {
    ...
    Unavailable: ['var(--primary)', 'var(--border)',  'var(--border)'],
  };
  ...
  const interviewInfo = (status === 'Interview' && c.interview_date) ? `
    <div style="background:rgba(106,13,173,0.06);border:1px solid rgba(106,13,173,0.15);border-radius:var(--radius-sm);padding:14px;margin-bottom:12px">
      ...
      ${c.interviewing_mentor_name ? `<div style="margin-top:8px;font-size:12px;color:var(--muted)">👤 Ditangani oleh: <strong>${c.interviewing_mentor_name}</strong></div>` : ''}
    </div>` : '';

  const mentorLockInfo = (lockedByOther && status === 'Pending') ? `
    <div style="background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.2);border-radius:var(--radius-sm);padding:12px 14px;margin-bottom:12px;font-size:13px">
      👤 Sedang ditangani oleh: <strong>${c.interviewing_mentor_name || 'Mentor lain'}</strong>
    </div>` : '';
  ...
  if (isUnavailable) {
    actionButtons = `
      <div style="margin-top:16px;padding:14px 16px;background:rgba(107,114,128,0.08);border:1px solid rgba(107,114,128,0.25);border-radius:var(--radius-sm);text-align:center;font-size:13px;color:#6b7280">
        🚫 Kandidat ini sudah <strong>diterima sebagai Intern di posisi lain</strong>. Tidak dapat diproses lebih lanjut.
      </div>`;
  } else if (lockedByOther) {
    actionButtons = `
      <div style="margin-top:16px;padding:14px 16px;background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.25);border-radius:var(--radius-sm);text-align:center;font-size:13px;color:#b45309">
        🔒 Kandidat ini sedang diproses oleh <strong>${c.interviewing_mentor_name || 'mentor lain'}</strong>
      </div>`;
  ...
  } else if (status === 'Interview' && (lockedByMe || !c.interviewing_mentor_id)) {
    actionButtons = `
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:20px;padding-top:20px;border-top:1px solid var(--border)">
  ...
  if (btnCallInterview) {
    btnCallInterview.addEventListener('click', () => {
      closeModal();
      openInterviewScheduleModal(c.id, c.name);
    });
  }
...
function openInterviewScheduleModal(candidateId, candidateName) {
  ...
  openModal(`
    ...
    <div style="margin-top:12px;padding:10px 14px;background:rgba(106,13,173,0.06);border-radius:var(--radius-sm);font-size:12px;color:var(--muted)">
      ℹ️ Kandidat ini akan terkunci untuk Anda dan tidak bisa diproses mentor lain setelah dipanggil interview.
    </div>
...
async function updateCandidateStatus(id, status, interviewDetails, assignedMentorId = '') {
  ...
  if (res.success) {
    const labels = {
      Interview: '📞 Kandidat dipanggil interview! Kandidat terkunci untuk Anda.',
